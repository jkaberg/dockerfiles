...
def check_requirements():
    ...
    # Add saslauthd check if we have SMTP users
    config = from_environment()
    if config.smtp.smtp_auth_enabled:
        try:
            subprocess.run(["which", "saslauthd"], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        except subprocess.CalledProcessError:
            logger.error("Required command saslauthd not found but SMTP authentication is enabled")
            return False
...
def start_services():
    ...
    try:
        subprocess.run(["service", "rsyslog", "start"], check=True)
        logger.info("Started rsyslog service")
    except subprocess.CalledProcessError as e:
        logger.error(f"Failed to start rsyslog: {e}")
    
    # Start OpenDKIM if enabled
    if config.dkim.enabled:
        try:
            subprocess.run(["service", "opendkim", "start"], check=True)
    ...
    if config.security.fail2ban_enabled:
        try:
            subprocess.run(["service", "fail2ban", "start"], check=True)
            logger.info("Started Fail2ban service")
        except subprocess.CalledProcessError as e:
            logger.error(f"Failed to start Fail2ban: {e}")
    
    # Start PostSRSD if enabled
    if config.srs.enabled:
        try:
            subprocess.run(["service", "postsrsd", "start"], check=True)
            logger.info("Started PostSRSD service")
        except subprocess.CalledProcessError as e:
            logger.error(f"Failed to start PostSRSD: {e}")
    
    # Start Cyrus SASL if SMTP authentication is enabled
    if config.smtp.smtp_auth_enabled:
        os.makedirs("/var/run/saslauthd", exist_ok=True)
        try:
            subprocess.run(["service", "saslauthd", "start"], check=True)
            logger.info("Started Cyrus SASL authentication service")
        except subprocess.CalledProcessError as e:
            logger.error(f"Failed to start Cyrus SASL: {e}")
    
    # Start Postfix
    try:
        subprocess.run(["service", "postfix", "start"], check=True)
        logger.info("Started Postfix service")
    except subprocess.CalledProcessError as e:
        logger.error(f"Failed to start Postfix: {e}")
